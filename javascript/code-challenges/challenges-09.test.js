...
// Do not understand it, found it at https://dirask.com/posts/JavaScript-find-max-value-in-array-with-Array-reduce-method-BDdeQp
// I just dont get reduce at all
...
const updateNumbers = (obj) => Object.entries(obj).map(([name, number]) => `${name}: ${number}`);
...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  let firstName = arr.find((obj) => obj.name === character);

  if (firstName) {
    let data = Object.values(firstName);
    return data.includes(firstName.children);
  }
  return false;
};
