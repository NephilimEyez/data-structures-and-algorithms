...
function transformToLis(obj){
  // Solution code here...
  let result = Object.entries(obj).map(entry => `<li>${entry.join(': ')}</li>`);
  return result;
}
...
const count = (target, input) => {
  let count = 0;
  input.forEach(arr => {
    count += arr.reduce((acc, curr) => curr === target ? acc + 1 : acc, 0);
  });
  return count;
};
...
const divisibleByFiveTwoToThePower = (input) => input.map((arr) => arr.filter((num) => typeof num === 'number' && num % 5 === 0).map((num) => Math.pow(2, num)));
...
let findShortest = (data) => data.filter((character) => character.height !== 'unknown').reduce((shortest, character) => parseInt(character.height) < parseInt(shortest.height) ? character : shortest).name;
